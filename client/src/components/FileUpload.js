...
const FileUpload = ({ contract, account, provider }) => {
  ...
  const [fileName, setFileName] = useState("No image selected");
        ...
        const apike = process.env.REACT_APP_API_KEY;
        const apipk = process.env.REACT_APP_PINATA_API_SECRET_KEY;
    ...
    if (file) {
      try {
        ...
        const resFile = await axios({
          ...
          data: formData,
          headers: {
            pinata_api_key:apike.toString() ,
            pinata_secret_api_key : apipk.toString(),
